...
files = lapply(1:10, function(v) {
  ...
  filter(downsampling) # IPW will only affect downsampling
...
future.apply::future_lapply(1:nrow(files), function(ctr) {
  ...
  if (downsampling) {
    ...
    #inverse probabiliy weighting
    ctr_prob  = sum(!phen$status) / sum(cur_true$status == FALSE)
    case_prob = sum(phen$status)  / sum(cur_true$status == TRUE)
    IPW = ifelse(phen$status, 1/case_prob, 1/ctr_prob)
  } else {
    KEEP = 1:nrow(G)
    phen = fread(str_subset(files$grps[[ctr]], "true")) %>% 
      as_tibble %>% 
  ...
  res_files = paste0("./gwas-results/v",v, "_PHModel_IPW_N1000k_M20k_", 
                     gen_beta, "_", 
                     cur_C, "_", 
                     prev, 
  ...
  all_gwas_parts = lapply(1:nrow(intervals), function(ic) {
    ...
    big_apply(X = G, a.FUN = function(X, ind, analysed_snps, phen, ind_keep, IPW) {
      library(dplyr)
      library(survival)
      ...
      lapply(as_tibble(tmp_X), function(cur_snp) {
        ...
        PHModel = coxph(Surv(event, status) ~ cur_snp, phen2, weights = IPW)
        summary(PHModel)$coefficients[6]
      }) %>% unlist()
      
    }, ncores = NCORES, analysed_snps = analysed_snps, phen = phen, ind = ind, ind_keep = KEEP, IPW = IPW) %>% 
      do.call("c", .)
